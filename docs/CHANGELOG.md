...
- 新增 `yistackctl ephemeral enable|disable`，统一完成无痕体验模式的基线采集、配置启停和 systemd timer 管理；启用时即使已有普通用户和项目，也会生成排除这些数据的干净基线。
- 管理后台删除普通用户改为事务化物理删除，并同步清理所属项目的容器、网络、终端会话、工作区、本地及远端备份和关联数据库记录；任一资源清理失败都会阻止数据库物理删除；远端备份先进入可恢复对象存储隔离区，数据库删除失败或服务重启时自动恢复。

### 变更

- 无痕体验模式状态新增完整基线兼容性以及两个 timer 的运行、启用状态；升级会以 `EPHEMERAL_MAINTENANCE_ENABLED` 为真源，原先启用时自动为新 Release 重建干净基线并恢复 timer，原先关闭时保持正常模式。升级失败回滚时恢复旧配置、旧基线和 timer 状态。
- 每日无痕重置新增本地项目备份清理，普通用户数据不会被写入新基线，启用动作本身不会删除当前在线数据。
- 中英文 README 按启用、停用、修改配置、重新启用和升级后处理重写无痕体验模式说明。
- 方案生成的流式分析与候选方案统一要求面向用户的说明文字使用简体中文；管理员自定义 Prompt 不再覆盖该语言与阶段输出协议。
- 永久删除用户会阻止新的用户请求和项目写入，取消在途生成与运行时准备任务，并暂存工作区和本地备份；数据库删除失败时恢复暂存数据，避免并发残留和部分删除。
- 项目部署、回滚、域名和发布读取统一进入项目删除屏障；项目进入破坏性清理后，资源清理或最终硬删除失败会保持软删除并持续重试，不再恢复已部分清理的项目。
- 无痕模式启停会先取得维护锁再停止 systemd 维护服务，避免并发 `reset` 在数据库恢复中途被终止。
- 升级与无痕维护共享同一把维护锁；`enable` 失败会恢复旧基线。项目元数据更新、停止容器、远端备份上传和资源告警副作用统一进入用户与项目删除屏障。告警通知与硬配额执行新增数据库级原子 claim，并与 pending 意图在同一事务中落库，避免多实例重复副作用或失败 claim 永久阻断；模板创建失败的补偿删除会保留并报告数据库与工作区恢复错误。
- 资源告警 webhook 与硬配额执行会先写入 pending 意图；外部结果不确定或成功结果落库失败时禁止自动重放，避免重复通知或重复停止容器。
...
- Playwright 安装会检查当前版本要求的 Chromium、Headless Shell 和 FFmpeg revision，并明确报告复用缓存或下载缺失组件。
- 无痕模式的每小时任务改为仅检查磁盘水位，低于高水位时不再执行 TTL 删除；每日重置继续负责完整清理，TTL 清理保留为显式人工命令。
- `uninstall --purge` 会终止并等待服务用户进程退出，完成账号清理后才删除本地文件；进程或账号清理失败时保留数据供重试，避免留下账号与数据状态不一致的半卸载结果。
