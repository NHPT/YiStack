...
export async function GET(request:NextRequest,{params}:{params:Promise<{templateId:string}>}){const {templateId}=await params;return proxyBackendRequest(request,{method:'GET',backendPath:`/api/admin/project-templates/${encodeURIComponent(templateId)}/versions`,errorBody:(error)=>buildBackendProxyErrorBody('official template versions',error)});}
