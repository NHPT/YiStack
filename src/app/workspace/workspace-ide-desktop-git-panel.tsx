...
function getGitPanelRenderableSelectedCommitDetail(selectedCommit: GitCommit | null): GitCommit | null {
  if (selectedCommit === null) {
    return null;
  }
...
function canRenderGitPanelSelectedCommitDetailStaleBanner(
  status: GitPanelCommitDetailStatus | null,
  ...
  return canRenderGitPanelCommitDetailStaleBanner(status, selectedCommit);
}
...
function materializeGitPanelCommitNodes({
  ...
  for (const commit of commits) {
    ...
    const commitRowClassName = getGitPanelCommitRowClassName(
      'w-full border-b border-transparent p-3 text-left transition-colors hover:bg-muted/50',
      isSelected,
    );
    ...
    nodes.push(
      <button
        key={commitRowKey}
        onClick={() => void onViewCommit(commit)}
        className={commitRowClassName}
      >
        ...
        <p className="mt-1 text-xs text-muted-foreground">{commit.files} 个文件变更</p>
...
function materializeGitPanelBranchCompareCommitNodes({
  ...
  for (const commit of commits) {
    ...
    nodes.push(
      <div key={commit.hash} className="flex items-center gap-2 rounded border bg-background/50 px-2 py-1">
        ...
        <span className="hidden shrink-0 text-muted-foreground sm:inline">{branchCompareCommitAuthorLabel}</span>
        <Button
          size="sm"
          variant="ghost"
          className="h-7 shrink-0 text-xs"
...
function materializeGitPanelBranchCompareFileNodes({
  ...
  for (const file of files) {
    ...
    nodes.push(
      <div key={file.path} className="flex items-center gap-2 rounded border bg-background/50 px-2 py-1">
...
function materializeGitPanelWorktreeDiffFileNodes({
  ...
  for (const file of files) {
    ...
    const diffLineNodes = materializeGitPanelDiffLineNodes({
      ...
      lineBaseClassName: '-mx-2 rounded px-2 py-0.5',
    });

    nodes.push(
      <AccordionItem
        key={diffFileItemKey}
        value={`worktree-${diffFileItemKey}`}
        className="overflow-hidden rounded-lg border bg-card"
        data-testid="workspace-git-worktree-diff-preview"
      >
        <AccordionTrigger className="bg-muted/30 px-3 py-2 hover:no-underline">
          <div className="mr-4 flex flex-1 items-center justify-between">
            <span className="flex min-w-0 items-center gap-2 truncate text-sm font-medium">
              <FileCode className="h-3.5 w-3.5 shrink-0 text-muted-foreground" />
              {gitDiffFileItemSnapshot.path}
            </span>
        ...
        <AccordionContent>
          ...
          <pre className="overflow-x-auto bg-muted/20 p-3 text-xs font-mono leading-relaxed">
            {diffLineNodes}
          </pre>
        </AccordionContent>
      </AccordionItem>,
...
function materializeGitPanelSelectedCommitDiffFileNodes({
  commit,
  files,
  onOpenFile,
  onCopyText,
  ...
  for (const file of files) {
    ...
    nodes.push(
      <AccordionItem key={diffFileItemKey} value={`file-${diffFileItemKey}`} className="overflow-hidden rounded-lg border bg-card">
        <AccordionTrigger className="bg-muted/30 px-3 py-2 hover:no-underline">
          <div className="mr-4 flex flex-1 items-center justify-between">
            <span className="truncate text-sm font-medium">{gitDiffFileItemSnapshot.path}</span>
            <div className="ml-4 flex shrink-0 items-center gap-3 text-xs">
              <span className="text-emerald-600 dark:text-emerald-400">+{file.additions}</span>
              <span className="text-red-600 dark:text-red-400">-{file.deletions}</span>
            </div>
...
export function DesktopGitPanel({
  projectId,
  gitBranch,
  gitBranches,
  gitBranchListStatus,
  ...
  const renderableSelectedCommitDetail = getGitPanelRenderableSelectedCommitDetail(selectedCommit);
  const canRenderSelectedCommitDetailStaleBanner = canRenderGitPanelSelectedCommitDetailStaleBanner(
    gitCommitDetailStatus,
    renderableSelectedCommitDetail,
  );
  ...
  return (
    <div className="h-full flex">
          ...
          <AccordionItem value="advanced-git-operations" className="rounded-md border bg-muted/10">
            ...
            <AccordionContent className="pb-3">
              ...
              {canRenderWorktreeDiffFiles === true && (
                <Accordion type="multiple" className="mx-3 mt-3 space-y-2">
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        {canRenderGitPanelSelectedCommitDetail(renderableSelectedCommitDetail) === true ? (
          <>
            {(() => {
              const selectedCommitDetail = renderableSelectedCommitDetail;
              const selectedCommitDiffFiles = getGitPanelCommitDiffFiles(selectedCommitDetail);
              const selectedCommitDiffFileCount = getGitPanelCollectionItemCount(selectedCommitDiffFiles);
              const canRenderSelectedCommitDiffFiles = canRenderGitPanelCollection(selectedCommitDiffFileCount);

              return (
                <>
            <div className="min-h-[100px] shrink-0 border-b bg-muted/10 p-4">
              {canRenderSelectedCommitDetailStaleBanner === true && (
              ...
              <div className="mb-2 flex items-center gap-2">
                ...
                <code className="rounded bg-muted px-2 py-1 text-sm font-mono">{selectedCommitDetail.hash}</code>
              </div>
              <h3 className="mb-2 text-base font-medium">{selectedCommitDetail.message}</h3>
              <div className="flex items-center gap-4 text-xs text-muted-foreground">
                <span>{selectedCommitDetail.author}</span>
                <span>{selectedCommitDetail.email}</span>
                <span>{selectedCommitDetail.time}</span>
              </div>
            </div>
            <div className="flex-1 overflow-y-auto">
              {canRenderSelectedCommitDiffFiles === true ? (
                <Accordion type="multiple" className="space-y-2 p-4">
                  {materializeGitPanelSelectedCommitDiffFileNodes({
                    commit: selectedCommitDetail,
                    files: selectedCommitDiffFiles,
                    onOpenFile,
                    onCopyText,
                    setPendingCommitFileRestoreConfirmation,
                  })}
          ...
          <div className="h-full flex items-center justify-center text-muted-foreground">
            <div className="max-w-xl px-4 text-center">
              <GitBranchIcon className="mx-auto mb-3 h-12 w-12 opacity-50" />
              <p className="text-sm">选择一个提交查看详情</p>
