...
assert.match(
  workspaceTypesSource,
  /export type ChatAttachmentSnapshotStatus = 'empty' \| 'selected' \| 'removed' \| 'picker_empty' \| 'rejected';[\s\S]*export type ChatAttachmentSnapshotSource = 'attachment_state' \| 'file_picker' \| 'clipboard' \| 'user_action';[\s\S]*export type ChatAttachmentSnapshot = \{[\s\S]*status: ChatAttachmentSnapshotStatus;[\s\S]*source: ChatAttachmentSnapshotSource;[\s\S]*attachmentCount: number;[\s\S]*totalSize: number;[\s\S]*lastFileName: string \| null;[\s\S]*recovery: string;/,
...
assert.match(
  ...
  /(?=[\s\S]*ChatInputSnapshotStatus)(?=[\s\S]*ChatInputSnapshotSource)(?=[\s\S]*export function buildChatInputSnapshot\([\s\S]*\): ChatInputSnapshot \{)(?=[\s\S]*const hasPrompt = promptLength > 0;)(?=[\s\S]*const hasAttachments = attachmentCount > 0;)(?=[\s\S]*const hasContent = hasPrompt === true \|\| hasAttachments === true;)(?=[\s\S]*const hasSelectedModel = selectedModel\.length > 0;)(?=[\s\S]*const canSendBase = planSelectionPending === false[\s\S]*hasContent === true[\s\S]*isBusyGenerating === false;)(?=[\s\S]*canSend: canSendBase)(?=[\s\S]*selectedModel: hasSelectedModel === true \? selectedModel : 'default')(?=[\s\S]*updatedAt: 'derived')(?=[\s\S]*if \(isStopConfirming === true\)[\s\S]*status: ChatInputSnapshotStatus = 'stop_confirmation'[\s\S]*source: ChatInputSnapshotSource = 'stop_control')(?=[\s\S]*if \(isGenerating === true\)[\s\S]*status: ChatInputSnapshotStatus = 'generating'[\s\S]*source: ChatInputSnapshotSource = 'generation_state')(?=[\s\S]*if \(isPlanning === true\)[\s\S]*status: ChatInputSnapshotStatus = 'planning'[\s\S]*source: ChatInputSnapshotSource = 'generation_state')(?=[\s\S]*if \(planSelectionPending === true\)[\s\S]*status: ChatInputSnapshotStatus = 'plan_selection_required'[\s\S]*source: ChatInputSnapshotSource = 'plan_selection')(?=[\s\S]*if \(hasContent === false\)[\s\S]*status: ChatInputSnapshotStatus = 'empty_prompt'[\s\S]*source: ChatInputSnapshotSource = 'input_buffer')(?=[\s\S]*status: ChatInputSnapshotStatus = 'model_unconfigured'[\s\S]*source: ChatInputSnapshotSource = 'model_registry')(?=[\s\S]*status: ChatInputSnapshotStatus = 'ready_to_send'[\s\S]*source: ChatInputSnapshotSource = 'input_buffer')/,
