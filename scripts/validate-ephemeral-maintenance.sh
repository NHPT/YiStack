...
grep -q '^schema=ephemeral-experience-baseline.v1$' "$TEMP_ROOT/data/ephemeral-baseline/METADATA" ||
  fail "snapshot did not write the ephemeral experience baseline schema"
grep -q '^user_data_policy=empty$' "$TEMP_ROOT/data/ephemeral-baseline/METADATA" ||
