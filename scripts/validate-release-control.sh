...
postgres_start_output="$(run_control postgres start)"
grep -Fqx 'YiStack PostgreSQL started and is ready.' <<< "$postgres_start_output"
grep -Fqx 'start yistack-postgres.service' "$root/systemctl.log"
postgres_restart_output="$(run_control postgres restart)"
grep -Fqx 'YiStack PostgreSQL restarted and is ready.' <<< "$postgres_restart_output"
grep -Fqx 'restart yistack-postgres.service' "$root/systemctl.log"
postgres_stop_output="$(run_control postgres stop)"
grep -Fqx 'YiStack PostgreSQL stopped.' <<< "$postgres_stop_output"
grep -Fqx 'stop yistack-postgres.service' "$root/systemctl.log"
