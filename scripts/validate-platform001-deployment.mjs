...
assert.match(service,/func \(s \*ProjectDeploymentService\) beginProjectOperation[\s\S]*BeginCancellableUserProjectMutation/);
for(const operation of ['ListReleases','Deploy','RefreshRelease','ReleaseLogs','Rollback','ListDomains','mutateDomain']) assert.match(service,new RegExp(`func \\(s \\*ProjectDeploymentService\\) ${operation}\\([\\s\\S]*?beginProjectOperation`),`${operation} must enter the project deletion gate`);
...
for(const name of ['TestProjectDeploymentDeployEncryptsSecretsAndReplaysIdempotently','TestProjectDeploymentLogsRollbackAndDomainLifecycle','TestProjectDeploymentValidationFailureBlocksProviderMutation','TestVercelAdapterUploadsImmutableArtifactWithoutTokenInURL','TestProjectDeploymentOperationsRejectedDuringProjectDeletion','TestProjectDeletionCancelsActiveDeployment']) assert.ok(tests.includes(name),`missing ${name}`);
